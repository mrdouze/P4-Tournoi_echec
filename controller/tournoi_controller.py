...
class GenererTournoi:
    ...
    def creation_tournoi(cls):
        ...
        if reponse == 'o':
            ...
            Database.inserer_joueur(joueur_serialise)
            rang = random.randrange(1, 1000)
            joueur = Joueur('Lacaisse', 'Alphonse', '06/04/78', 'm', rang, 0)
            tournoi.ajouter_joueur(joueur)
            joueur_serialise = Database.serialiser_joueur(joueur)
            Database.inserer_joueur(joueur_serialise)
            rang = random.randrange(1, 1000)
            joueur = Joueur('Penflam', 'Kathy', '06/06/90', 'f', rang, 0)
            tournoi.ajouter_joueur(joueur)
            joueur_serialise = Database.serialiser_joueur(joueur)
            Database.inserer_joueur(joueur_serialise)
            rang = random.randrange(1, 1000)
            joueur = Joueur('Honeth', 'Camille', '01/02/98', 'f', rang, 0)
            tournoi.ajouter_joueur(joueur)
            joueur_serialise = Database.serialiser_joueur(joueur)
            Database.inserer_joueur(joueur_serialise)
            rang = random.randrange(1, 1000)
            joueur = Joueur('Fonfek', 'Sophie', '23/08/64', 'f', rang, 0)
            tournoi.ajouter_joueur(joueur)
            joueur_serialise = Database.serialiser_joueur(joueur)
            Database.inserer_joueur(joueur_serialise)
            rang = random.randrange(1, 1000)
            joueur = Joueur('Celere', 'Jacques', '19/06/65', 'm', rang, 0)
            tournoi.ajouter_joueur(joueur)
            joueur_serialise = Database.serialiser_joueur(joueur)
            Database.inserer_joueur(joueur_serialise)
